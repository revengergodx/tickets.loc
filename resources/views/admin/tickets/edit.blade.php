                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('admin.ticket.index') }}">Ticket</a></li>
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <form
                            action="{{ route('admin.ticket.update', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="form-group col-4">
                                <label>Оберіть Агента</label>
                                <select name="agent_id" class="form-control">
                                    @foreach($agents as $agent)
                                    <option value="{{ $agent->id }}" {{ $agent->id == $ticket->agent_id ? 'selected' : ''}}>{{ $agent->name }}</option>
                                    @endforeach
                                    @error('tag_ids')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="form-group col-4">
                                <label>Статус : {{ $ticket->status }}</label>
                    ...
                    <div class="col-12">
                        @if(count($ticket->images)>0)
                            ...
                            @foreach($ticket->images as $img)
                                <form action="{{route('admin.ticket.delete_image', $img->id)}}" method="post">
                                    @csrf
                                    @method('delete')
